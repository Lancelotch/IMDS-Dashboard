...
import {
 ...
 IPayloadEditInternalUser,
 IPayloadSuspenseUser,
 IResponseAddInternalUser,
 IResponseChangePassword,
 IResponseInternalUserById,
 IResponseInternalUserList
...
export const useInternalUser = () => {
 ...
 const editInternalUser = async (
  id: string,
  payload: IPayloadEditInternalUser
 ) => {
  dispatch(reducerUpdateLoadingInternalUser(true));
  try {
   const response = await httpClient.put<IResponseAddInternalUser>(
    `/internal_user/update/${id}`,
    payload
   );
   if (response.status === 200) {
    dispatch(reducerEditInternalUser(response.data.data));
 ...
 const suspenseInternalUser = async (
  id: string,
  payload: IPayloadSuspenseUser
 ) => {
  dispatch(reducerUpdateLoadingInternalUser(true));
  try {
   const response = await httpClient.put<IResponseAddInternalUser>(
    `/internal_user/suspend/${id}`,
    payload
   );
   if (response.status === 200) {
    dispatch(reducerEditInternalUser(response.data.data));
 ...
 const changePassword = async (id: string, payload: IPayloadChangePassword) => {
  dispatch(reducerUpdateLoadingInternalUser(true));
  try {
   const response = await httpClient.put<IResponseChangePassword>(
    `/internal_user/change_password/${id}`,
    payload
   );
   if (response.status === 200) {
    handleClickAlert({
 ...
 const getInternalUserList = async (params: IPayloadGetList) => {
  dispatch(reducerUpdateLoadingInternalUser(true));
  try {
   const response = await httpClient.get<IResponseInternalUserList>(
    '/internal_user/find_all',
    { params }
   );
   if (response.status === 200) {
 ...
 const getInternalUserById = async (id: string) => {
  dispatch(reducerUpdateLoadingInternalUser(true));
  try {
   const response = await httpClient.get<IResponseInternalUserById>(
    `/internal_user/find_by_internal_user_id/${id}`
   );
   if (response.status === 200) {
